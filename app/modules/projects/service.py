...
class ProjectService:
    def __init__(self, session: AsyncSession, project_repository: ProjectRepository, user_repository: UserRepository):
    ...
    async def update(self, project_id: int, data: ProjectUpdateRequest) -> Project:
    ...
    async def delete(self, project_id: int) -> None:
        ...
        if await self.project_repository.is_used_in_reports(project_id):
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="This project cannot be deleted because it is used in report history.",
            )

        await self.project_repository.delete(project)
        await self.session.commit()

    async def assign_members(self, project_id: int, user_ids: list[int]) -> Project:
        project = await self.get_by_id(project_id)
        users = []
